...
<script>
  ...
  var btbe_cover = "";
	...
	var currentimgid = "";
jQuery('.img_upload').live('click', function( event ){
...
<h4 class="btbe_delete"><?php echo __('Are you absolutely sure you want to delete this book?', 'author-showcase'); ?> <a href="<?php echo admin_url('admin.php?page=btbe_add&btbe_id='.$_REQUEST['btbe_id'].'&realdelete=1'); ?>" class="button"><?php echo __('Yes, delete it!', 'author-showcase'); ?></a></h4>
<?php endif; ?>
<form action="" method="post">
  <div class="wrap">
    <table class="form-table btbe_add">
				...
				<?php endif; ?>
        <tr valign="top"><th scope="row"><?php echo __('Book Title', 'author-showcase'); ?></th>
            <td><input type="text" name="title" id="btbe_title" value="<?php echo stripslashes(@$_REQUEST['title']); ?>" /></td>
        </tr>
        <tr valign="top"><th scope="row"><?php echo __('Subtitle', 'author-showcase'); ?></th>
            <td><input type="text" name="subtitle" value="<?php echo stripslashes( @$_REQUEST['subtitle']); ?>" /></td>
        </tr>
        <?php if(!empty($series)) : ?>
        <tr valign="top"><th scope="row"><?php echo __('Series (Create a new series, or add to an existing series)', 'author-showcase'); ?></th>
            <td><input type="text" id="btbe_series" name="series" value="<?php echo stripslashes(@$_REQUEST['series']); ?>" />
        ...
        <?php else : ?>
        <tr valign="top"><th scope="row"><?php echo __('Series', 'author-showcase'); ?></th>
            <td><input type="text" name="series" value="<?php echo stripslashes(@$_REQUEST['series']); ?>" /></td>
        </tr>
        <?php endif; ?>
				<tr valign="top"><th scope="row"><?php echo __('Series Number (ex. 1, 2, #1, #2, One, Two)', 'author-showcase'); ?></th>
            <td><input type="text" name="series_num" value="<?php echo stripslashes(@$_REQUEST['series_num']); ?>" /></td>
        </tr>
        <tr valign="top"><th scope="row"><?php echo __('Author', 'author-showcase'); ?></th>
            <td><input type="text" name="author" value="<?php echo stripslashes(@$_REQUEST['author']); ?>" /></td>
        </tr>
				<tr valign="top"><th scope="row"><?php echo __('Short Blurb', 'author-showcase'); ?> <br /><small><?php echo __('(This is used in the sidebar and the list format - limit 400 characters)', 'author-showcase'); ?></small></th>
            <td><textarea name="short_blurb" rows="5"><?php echo stripslashes(@$_REQUEST['short_blurb']); ?></textarea></td>
        </tr>
        <tr valign="top"><th scope="row"><?php echo __('Blurb', 'author-showcase'); ?> <br /><small><?php echo __('(This is used in the single format)', 'author-showcase'); ?></small></th>
            <td><textarea name="blurb" rows="5"><?php echo stripslashes(@$_REQUEST['blurb']); ?></textarea></td>
        </tr>
				<tr valign="top"><th scope="row"><?php echo __('Link to a Book Page?', 'author-showcase'); ?> <br /><small><?php echo __('(This is used in the sidebar and the grid format slideout menus)', 'author-showcase'); ?></small></th>
			...
			<td><?php wp_dropdown_pages(array('name' => 'book_page', 'selected' => @$_REQUEST['book_page'], 'show_option_none' => $selectpage)); ?></td>
				...
				<tr valign="top"><th scope="row"><?php echo __('ASIN', 'author-showcase'); ?></th>
            <td><input type="text" name="asin" value="<?php echo stripslashes(@$_REQUEST['asin']); ?>" /></td>
        </tr>
				<tr valign="top"><th scope="row"><?php echo __('ISBN', 'author-showcase'); ?></th>
            <td><input type="text" name="isbn" value="<?php echo stripslashes(@$_REQUEST['isbn']); ?>" /></td>
        </tr>
        <tr valign="top"><th scope="row"><?php echo __('Cover Image', 'author-showcase'); ?> <br /><small><?php echo __('(The book title is used as alt text on the image)', 'author-showcase'); ?></small></th>
            <td><label for="upload_image">
                    <input id="cover_image" type="hidden" name="cover" value="<?php echo @$_REQUEST['cover']; ?>" /> 
                    <button id="cover_image_button" class="button img_upload"><?php echo __('Upload Image', 'author-showcase'); ?></button>
                </label>
            </td>
        </tr>
        <tr valign="top"><th scope="row"><?php echo __('Sales Links', 'author-showcase'); ?><br /><small><?php echo __('(Click add to create more)', 'author-showcase'); ?></small></th></tr>
		<tr>
            <td colspan="2">
					...
					<td>
						<p class="submit">
							<input type="hidden" name="btbe_add" value="1" />
								<input type="submit" class="button-primary" value="<?php echo __('Save', 'author-showcase'); ?>" onclick="btbe_checktitle(event)" />
						</p>
					</td>
					<td><p class="submit"><a href="<?php echo admin_url('admin.php?page=btbe_admin'); ?>" class="button"><?php echo __('Cancel', 'author-showcase'); ?></a></p></td>
				</tr>
				<?php if(@$_REQUEST['btbe_id'] > 0) : ?>
				<tr>
					<td><p class="submit"><a href="<?php echo admin_url('admin.php?page=btbe_add&btbe_id='.$_REQUEST['btbe_id'].'&delete=1'); ?>"><?php echo __('Delete this book', 'author-showcase'); ?></a></p></td>
				</tr>
				<?php endif; ?>
    </table>
    <div id="btbe_cover"><?php if(@$_REQUEST['cover'] != "") : ?><img src="<?php echo @$_REQUEST['cover']; ?>" /><?php endif; ?></div>
  <div class="clear"></div>
  </div>
</form>